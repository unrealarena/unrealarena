...
static void CG_Say( const char *name, int clientNum, saymode_t mode, const char *text )
{
	...
	switch ( mode )
	{
		...
		case SAY_TPRIVMSG:
			...
			{
				trap_S_StartLocalSound( cgs.media.qTalkSound, soundChannel_t::CHAN_LOCAL_SOUND );
				break;
			}
			else if ( cgs.clientinfo[ clientNum ].team == TEAM_U )
			{
				trap_S_StartLocalSound( cgs.media.uTalkSound, soundChannel_t::CHAN_LOCAL_SOUND );
				break;
			}
#else
