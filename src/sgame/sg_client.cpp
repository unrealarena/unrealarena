...
gentity_t *G_SelectSpawnPoint(team_t team, vec3_t origin, vec3_t angles, const void *data)
{
	...
	switch (team)
	{
		...
		default:
			Com_Error(errorParm_t::ERR_DROP, "G_SelectSpawnPoint: invalid team (%d)", team);
	}
...
gentity_t *G_SelectQSpawnPoint(vec3_t origin, vec3_t angles, const void *data)
{
	...
	if (!numSpawnPoints)
	{
		...
		if (!spawnPoint)
		{
			Com_Error(errorParm_t::ERR_DROP, "No pos_player_spawn found");
		}
...
gentity_t *G_SelectSpectatorSpawnPoint(vec3_t origin, vec3_t angles)
{
	...
	if (!spawnPoint)
	{
		...
		if (!spawnPoint)
		{
			Com_Error(errorParm_t::ERR_DROP, "No pos_player_intermission or pos_player_spawn found");
		}
	}
